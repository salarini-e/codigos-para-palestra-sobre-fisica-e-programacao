...
def detect_motion(video_path):
    ...
    # Variável para indicar se a detecção de movimento está ativa
    motion_detected = False

    # Obtem o centro do objeto como pixel de referência
    M = None
    reference_point = None

    # Lista para armazenar a trajetória do objeto
    trajectory = []
    ...
    while cap.isOpened():
        ...
        if max_contour is not None:
            ...
            if M["m00"] != 0:
                ...
                # Ativa a detecção de movimento somente quando o objeto é detectado
                motion_detected = True
        else:
            # Desativa a detecção de movimento quando não há objeto detectado
            motion_detected = False

        # Adiciona o pixel de referência à trajetória, somente quando o objeto é detectado
        if motion_detected and reference_point is not None:
            trajectory.append(reference_point)

        # Desenha um círculo no pixel de referência para visualização
        if motion_detected and reference_point is not None:
            cv2.circle(frame1, reference_point, 5, (0, 255, 0), -1)

        # Mostra o vídeo com o círculo no pixel de referência
...
def calculate_velocity_and_acceleration(pixel_positions, time_interval):
    # Calcula a velocidade e a aceleração usando as posições do pixel de referência
    velocities = []
    accelerations = []

    for i in range(1, len(pixel_positions)):
        x1, y1 = pixel_positions[i-1]
        x2, y2 = pixel_positions[i]
...
# Imprime o pixel de referência
print("Pixel de Referência:", trajectory)

# Realiza o cálculo da velocidade e aceleração do objeto usando a trajetória
